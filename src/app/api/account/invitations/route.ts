...
// Base-URL resolution lives in src/lib/branding.ts (resolveAppUrl).
// Fail-safe: APP_URL env wins; otherwise the request's own host is
// used. A production deploy with no APP_URL configured throws —
// invite links must never point at a domain we don't control.
function getBaseUrl(request: Request): string {
  // Centralized in src/lib/branding.ts — explicit APP_URL wins, then
  // the request's own host. Never an external domain (the previous
  // hard-coded marketing fallback is gone: misconfiguration must fail
  // loudly, not leak invitees to a site we don't control).
...
export async function POST(request: Request) {
  try {
    ...
    // Plan enforcement (server-side): team_members cap counts current
    // members + outstanding invites so a burst of invite links can't
    // sneak past the limit. The friendly 402 maps to the upgrade CTA.
    try {

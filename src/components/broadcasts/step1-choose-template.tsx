...
export function Step1ChooseTemplate({ selectedTemplate, onSelect, onNext, onBack }: Step1Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-foreground"><T k="broadcasts_step1_choose_template.001" /></h2>
        <p className="mt-1 text-sm text-muted-foreground"><T k="broadcasts_step1_choose_template.004" /></p>
      </div>

      {templates.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-border bg-card/50">
          <FileText className="mb-2 h-8 w-8 text-muted-foreground" />
          <p className="text-sm text-muted-foreground"><T k="broadcasts_step1_choose_template.002" /></p>
          <p className="mt-1 text-xs text-muted-foreground"><T k="broadcasts_step1_choose_template.003" /></p>
        </div>
      ) : (
      ...
      <div className="flex items-center justify-between border-t border-border pt-4">
        <Button variant="outline" onClick={onBack} className="border-border text-muted-foreground"><T k="dashboard_automations_id_logs_page.004" /></Button>
        <Button
          onClick={onNext}
